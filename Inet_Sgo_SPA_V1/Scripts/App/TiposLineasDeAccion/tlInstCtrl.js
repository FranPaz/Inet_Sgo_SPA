inetApp.controller('tlInstCtrl', function ($scope, tlInstDataFactory, $stateParams, $state, listadoLineasInst, listadoCamposProg, infoCampoProg) {

    $scope.lineas = listadoLineasInst; // var donde voy a guardar todos los tipos de lineas de campos prog ya sea cuando se cargue la pagino o cuando agregue uno nuevo
    ...
    $scope.addLinea = function () {
        $scope.lineas = tlInstDataFactory.query({ prmIdCampoProg: $stateParams.campoProgId });
    };

    //funcion para guardar una nueva linea
    $scope.lineaAdd = function (linea) {
        linea.CampoProgramaticoId = infoCampoProg.Id;
        tlInstDataFactory.save(linea).$promise.then(
            function () {
                $scope.addLinea();
    ...
    $scope.detalle = function (linea) {
        ...
        $scope.infoLinea = tlInstDataFactory.get({ prmIdCampoProg: $stateParams.campoProgId, prmIdLinea: linea.Id });
    ...
    $scope.save = function (linea) {// guarda los cambios y llama a la funcion put de la api          
        tlInstDataFactory.update({ id: linea.Id }, linea).$promise.then(
                function () {
                    $scope.lineas = tlInstDataFactory.query({ prmIdCampoProg: $stateParams.campoProgId });
                    $scope.editValue = false;
    ...
    $scope.cancel = function () {
        $scope.infoLinea = tlInstDataFactory.get({ prmIdCampoProg: $stateParams.campoProgId, prmIdLinea: $scope.infoLinea.Id });
        $scope.editValue = false;
    };
    //#endregion
    ...
    $scope.delete = function (linea) {
        tlInstDataFactory.delete(linea).$promise.then(
            function () {
                $scope.lineas = tlInstDataFactory.query({ prmIdCampoProg: $stateParams.campoProgId });
                $scope.clear();
